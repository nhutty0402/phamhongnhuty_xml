...
if __name__ == "__main__":
    ...
    for label, xpath_expr in queries:
    ...
    if result:
        for i, elem in enumerate(result, 1):
            # In mã NV để verify (bạn có thể thay bằng TENNV nếu muốn tên)
            manv = elem.xpath("./*[local-name()='MANV']")
            tennv = elem.xpath("./*[local-name()='TENNV']")
            print(f"  {i}. {tennv[0].text} (Mã: {manv[0].text})")
    else:
